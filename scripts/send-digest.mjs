...
import { matchFor, notDismissed, pilotMatch } from '../lib/signals.mjs';
...
function html(items, profile) {
  ...
  return `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:520px;margin:0 auto;padding:24px">
    ...
    <div style="color:#75867f;font-size:12px;margin-bottom:18px">New windows in NYC · ${profile}</div>
...
for (const pref of everyone) {
  ...
  // register (PROFILES[profileKey] || PROFILES.explore) and the digest form is
  // on that page. Skipping them told them "you're on the list" and then sent
  // nothing, ever.
  const profile = pref.profile || 'explore';
  ...
  // Dismissed cards are filtered before the cut, so a dismissal does not
  // shorten the digest.
  const items = matchFor(feed, profile, { onlyNew: true, portfolio }).filter(notDismissed(pref)).slice(0, 6);
  if (!items.length) { skipped++; continue; }

  if (pref.channels?.slack) {
    const ok = await postToSlack(
      pref.channels.slack,
      digestBlocks(items, labelOf(profile), `${items.length} new window${items.length > 1 ? 's' : ''} today`),
...
for (const p of await allPilots()) {
  ...
  // "This list, every morning": the register the pilot was started on, in its
  // ZIPs (lib/signals.mjs pilotMatch). It used to read the ZIP off the wrapper
  // rather than the row, which kept nothing, and to run every no-trade pilot
  // as a facade engineer whatever list they had been looking at.
  const items = pilotMatch(feed, p).slice(0, 6);
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      ...mailHeaders(email),
      html:
        html(items, p.trade ? labelOf(p.trade) : 'your list') +
