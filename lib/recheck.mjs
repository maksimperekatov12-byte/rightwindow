...
//   held         a directory number found before the rule that has not yet
//                shown its tie: off the cards, still re-read every day
//   rejected     a directory number with nothing left that could show the tie
//
...
// "captcha" alone is on every contact form with reCAPTCHA, and Cloudflare's
// /cdn-cgi/challenge-platform/ script rides on ordinary pages too — matching
// it filed eleven firms' readable contact pages (29 cards) as walls on the
// first dry run, where they would have lapsed at the end of their grace with
// the number sitting on the page.
const BOT_WALL =
  /cf-browser-verification|cf_chl_opt|<title>\s*(just a moment|attention required|access denied|pardon our interruption)|request unsuccessful\. incapsula|captcha-delivery\.com|px-captcha|enable javascript and cookies to continue/i;
function unreadable(html) {
  if (BOT_WALL.test(html)) return 'a bot check, not the page';
  return null;
}
// Too little text to say anything is missing from it. "You need to enable
// JavaScript to run this app." is 46 characters; an app shell, a splash page
// or an interstitial can still carry the number in a tel: link, and then it
// confirms it — but a thin page never counts as the number gone.
const THIN = 120;
const thin = (html) => textOf(String(html).replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ')).trim().length < THIN;
...
export function pageGetter(fetchImpl = globalThis.fetch, opts = {}) {
  ...
  async function load(url) {
    ...
    try {
      ...
      return { ok: true, status: res.status, html, url: res.url || url, thin: thin(html) };
    } catch (e) {
...
async function checkOwn(entry, { company, address }, page, get, max) {
  ...
  while (queue.length && fetched < max) {
    ...
    if (r.ok) {
      // A page that redirected to another host is not the page the number
      // came from: it may confirm the number, it cannot say it has gone.
      const own = hostOf(r.url || step.url) === page.host && !r.thin;
      loaded.push({ ...r, exact: Boolean(step.exact && own) });
      if (step.exact && own) exactLoaded = true;
  ...
  // an affiliate's number is somebody else's by definition. And only the very
  // page the number was read from: a homepage that loads while the office
  // page times out shows the head office's line, and taking it would swap a
  // number nobody had shown to be gone.
  if (v.confidence === 'verified' && v.phone && !page.host.endsWith('.gov')) {
    const r = loaded.find((x) => x.exact);
    const found = r ? acceptPage({ company, address, url: r.url, page: r.html }) : null;
...
//
// Nothing here withdraws a number on one bad reading. A listing page that
// cannot be read (403, a timeout, a wall, a redirect elsewhere) is no evidence
// either way; one that loads and fails the rule may be a consent page or a
// layout on the day. So a listing is withdrawn only when nothing could ever
// show its tie (no listing named, no address in its note), or when the page
// was read and failed twice, at least 20 hours apart. Until then a listing
// found before the rule, which has never shown its tie, is held: off the
// cards, and read again every day.
const GOV_HOST = /(^|\.)(gov|cityofnewyork\.us|ny\.us)$|^data\.(cityofnewyork\.us|ny\.gov)$/i;
async function checkListed(entry, address, page, get) {
  const v = entry.value;
  // A city or state record filed under "listed" was matched on name and
  // business address when it was taken; it is not a directory, and the
  // directory rule is not its test.
  if (page && GOV_HOST.test(page.host)) return { outcome: 'unconfirmed', note: 'a government record, not a directory listing' };
  const legacy = (entry.at || 0) < RULE_AT && !entry.checkedAt;
  const noted = pageShowsAddress(v.source, address);
  if (page?.exact) {
    const r = await get(page.url);
    const url = r.url || page.url;
    if (r.ok && hostOf(url) === page.host) {
      ...
      let reason;
      if (!v.phone) {
        if (shows && showsEmail(r.html, v.email)) return { outcome: 'confirmed', url };
        reason = `the directory page (${page.host}) does not tie this inbox to the filing's address`;
      } else {
        const near = shows ? phoneNearAddress(text, phonesFrom(text), address) : null;
        // A directory vouches for a number, never for an inbox: a search today
        // takes none from it, so a confirmed listing keeps only its number.
        if (near === v.phone) return { outcome: 'confirmed', url, email: null };
        if (near) return { outcome: 'changed', phone: near, url, email: null };
        reason = shows
      ...
      // Read, and the rule failed. Once is a strike; the second, a day on,
      // withdraws it. A listing that has never met the rule leaves the cards
      // at the first strike; one that met it keeps serving until the second.
      // A thin page is no reading at all.
      if (!r.thin) return { outcome: legacy ? 'held' : 'absent', strike: true, reason };
    }
    if (!legacy) return { outcome: 'unreachable' };
    if (noted) return { outcome: 'unreachable', note: "kept on the filing's address in its source note" };
    return { outcome: 'held', reason: `the directory page (${page.host}) could not be read to show the tie` };
  }
  // No listing named: only a host, or prose. A listing found under today's
  // rule met it when it was taken; one found before it has only its source
  // note, and a note without the address will never show the tie.
  if (!legacy) return { outcome: 'unreachable' };
  if (noted) return { outcome: 'unreachable', note: "kept on the filing's address in its source note" };
  ...
  switch (res.outcome) {
    case 'confirmed':
      return {
        ...entry,
        checkedAt: now,
        lastCheck: stamp('confirmed'),
        value: { ...v, ...(res.url ? { url: res.url } : {}), ...('email' in res ? { email: res.email } : {}) },
      };
    case 'changed':
      return {
        ...entry,
        ...
        value: {
          ...
          phone: res.phone,
          ...(res.url ? { url: res.url } : {}),
          ...('email' in res ? { email: res.email } : {}),
          was: { phone: v.phone, until: now },
        },
      };
    case 'absent':
    case 'held': {
      // A strike is a page that was read and did not vouch for the number.
      // One is a page having a bad day; two, a day apart, is the number gone
      // from it. A held listing that could not be read has no strike to count.
      const struck = res.outcome === 'absent' || res.strike;
      const prevStruck = prev && (prev.outcome === 'absent' || (prev.outcome === 'held' && prev.strike));
      const since = struck && prevStruck ? prev.since || prev.at : now;
      if (struck && prevStruck && now - since >= DUE_MS) {
        const why = res.reason ? `${res.reason} (since ${day(since)})` : `no longer on its source page since ${day(since)}`;
        return tombstone(entry, why, res.reason ? 'rejected' : 'withdrawn', now, company);
      }
      return {
        ...entry,
        lastCheck: stamp(res.outcome, { ...(struck ? { since, strike: true } : {}), ...(res.reason ? { reason: res.reason } : {}) }),
      };
    }
    case 'rejected':
  ...
  const worker = async () => {
    while (next < batch.length && clock() < deadline) {
      ...
      results.push({
        ...
        reason: after.value.rejected || res.reason || res.note || null,
      });
    }
  };
...
export function standing(doc, pairs, now) {
  const s = { contacts: 0, confirmed: 0, changed: 0, absent: 0, held: 0, unconfirmed: 0, unreachable: 0, unchecked: 0, withdrawn: 0, due: 0 };
...
export async function runRecheck({ feed, readJson, writeJson, fetchImpl = globalThis.fetch, dryRun = false, now = Date.now(), limits = {} }) {
  ...
  if (!dryRun) {
    ...
    // A push that fails is not a read that failed: the checks are done and on
    // disk for push-contacts, and the store keeps its older copy until the
    // next run, which checks again.
    try {
      pushed = await pushCache(writeJson);
    } catch (e) {
      pushError = String(e?.message || e).slice(0, 160);
    }
  }
  return { registers, pairs, before, doc, ...run, ms, pushed, pushError, standing: standing(doc, pairs, now) };
}
