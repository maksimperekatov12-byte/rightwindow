...
const DATASET_URL = /\/(resource|api\/views)\/([a-z0-9]{4}-[a-z0-9]{4})/;
...
export function sourceKeyOf(url) {
  const m = String(url).match(DATASET_URL);
  if (!m) return null;
  for (const [key, s] of Object.entries(SOURCES)) if (s.id === m[2] || s.also?.includes(m[2])) return key;
  return null;
}

// What a request to a dataset read: its rows (/resource/<id>) or its metadata
// (/api/views/<id>.json), which is where the publisher's clock lives. The two
// land under the same record but answer different questions — a clock that
// answers says nothing about whether the rows did. Null for anything else.
export function readKindOf(url) {
  const m = String(url).match(DATASET_URL);
  return m ? (m[1] === 'resource' ? 'rows' : 'clock') : null;
}
