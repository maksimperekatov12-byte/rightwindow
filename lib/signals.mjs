...
// Mirrors the trade filters in src/App.jsx (cFilter / oFilter / fFilter): what a
// trade is emailed must be what it sees on the site.
...
export const contractMatch = {
  qewi: facadeBid,
  restoration: facadeBid,
  equipment: (c) => CONSTR.test(c.category || ''),
  insurance: (c) => c.kind === 'AWARD' && CONSTR.test(c.category || ''),
  lender: (c) => c.kind === 'AWARD',
  staffing: (c) => c.kind === 'AWARD',
};
export const openingMatch = {
  fnb: isVenue,
  marketing: isVenue,
};
...
export function matchFor(feed, profile, { onlyNew = true, portfolio = null } = {}) {
  ...
  if (OPENING_PROFILES.has(profile)) {
    const m = openingMatch[profile] || (() => true);
    for (const o of feed.openings) {
      if (o.src === 'sla' && NOT_A_VENUE.test(o.kind || '')) continue;
      if (!m(o) || (onlyNew && !o.isNew)) continue;
