...
function companyIndex() {
  ...
  byName = new Map();
  for (const entry of Object.values(loadCache())) {
    const v = entry?.value;
    if (!v || v.confidence === 'none' || (!v.phone && !v.email)) continue;
    const n = v.company ? nameKey(v.company) : null;
    if (n && !byName.has(n)) byName.set(n, entry);
  }
  return byName;
}
...
export function searchesToday() {
  ...
  for (const e of Object.values(loadCache())) if ((e?.at || 0) >= start.getTime()) stamps.add(e.at);
  return stamps.size;
}
...
export async function enrichContact({ company, name, address, cacheOnly = false } = {}) {
  ...
  // Same firm, different building on the filing.
  const sibling = companyIndex().get(nameKey(company));
  if (sibling && Date.now() - sibling.at < TTL_MS) {
    c[k] = { at: sibling.at, value: sibling.value };
    saveCache();
  ...
  if (value.unsearched) return EMPTY;
