...
// Whether a number is still one the cache serves for a firm, under any of its
// filing addresses. The collector's affiliate pass (lib/affiliate.mjs) hands a
// holding LLC the contact of the firm whose head officer signs for both, from
// the cache as it stood when the collector ran; the re-check runs after it and
// may withdraw that very number. push-contacts asks this before it publishes an
// inherited one, so a withdrawn number cannot come back under another name.
// The answer is the day the number was last seen good, or null.
export function stillServed(company, { phone = null, email = null } = {}, now = Date.now()) {
  const n = nameKey(company);
  if (!n || (!phone && !email)) return null;
  for (const e of Object.values(loadCache())) {
    const v = e?.value;
    if (!v || v.rejected || e.lastCheck?.outcome === 'held' || nameKey(v.company) !== n) continue;
    if (!((phone && v.phone === phone) || (!phone && email && v.email === email))) continue;
    const age = now - lastConfirmed(e);
    if (age < TTL_MS || (age < GRACE_MS && GRACE_OUTCOMES.has(e.lastCheck?.outcome))) {
      return new Date(lastConfirmed(e)).toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
    }
  }
  return null;
}
