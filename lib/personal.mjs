...
export function resolveIdentities(rows, opts = {}) {
  ...
  for (const r of rows) {
    ...
    if (r.nameShown) cost.shown++;
      ...
      cost.total++;
      r.identity = identityOf(r, addrOf(r));
    }
  }
  return { rows, cost };
}

// The address is the identity when the name cannot be. "A new food business at
// 62 Manhattan Ave" is still a lead; a stranger's name is not ours to print.
function identityOf(r, address) {
  const addr = String(address || '').split(',')[0].trim();
  return addr ? `New ${String(r.kind || 'business').toLowerCase()} at ${addr}` : 'New business, address on file';
}

/**
 * The five-minute lane's half of the same decision.
 *
 * That lane re-reads the liquour file between hourly builds and published
 * every licensee's name as it found it, so a name the hourly build had
 * withheld came back within five minutes — on the public data branch, through
 * /api/live, and onto the card, because the site lays the fast row over the
 * hourly one. Where the hourly build has judged a row (`hourlyRows`, its
 * published openings), its verdict is carried as it stands. A row it has not
 * seen yet is tested here with what this lane has — no DCWP premises set and no
 * other register to find a chain in — which can only err toward hiding; the
 * hourly build puts a vouched name back on its next run.
 *
 * A withheld name is deleted, not flagged: a name left on the row is a name
 * published.
 */
export function carryIdentities(rows, hourlyRows = []) {
  const hourly = new Map((hourlyRows || []).map((r) => [r.id, r]));
  for (const r of rows) {
    const h = hourly.get(r.id);
    // A "hide" verdict carries whatever the name now says — hiding is the safe
    // side. A "show" verdict vouched for one particular name: if the licence's
    // DBA changed since the hourly run, the new name has not been vetted.
    const judged =
      h && typeof h.nameShown === 'boolean' && (h.nameShown === false || String(h.name || '') === String(r.name || ''));
    r.evidence = judged ? h.evidence || [] : nameEvidence(r.name, { legalName: r.legal });
    r.nameShown = judged ? h.nameShown : r.evidence.length > 0;
    if (r.nameShown) {
      delete r.identity;
      continue;
    }
    delete r.name;
    delete r.legal;
    r.identity = identityOf(r, r.address);
  }
  return rows;
}
